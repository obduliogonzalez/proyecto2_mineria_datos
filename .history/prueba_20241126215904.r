...
multas$Sexo_inf <- as.factor(multas$Sexo_inf)
multas$Est_ebriedad_inf <- as.factor(multas$Est_ebriedad_inf)
multas$Área_geo_inf <- as.factor(multas$Área_geo_inf)

# Gráfico 1: Distribución de Estado de Ebriedad
g1 <- ggplot(multas, aes(x = Est_ebriedad_inf, fill = Est_ebriedad_inf)) +
  geom_bar() +
...
g2 <- ggplot(multas, aes(x = Sexo_inf, fill = Est_ebriedad_inf)) +
...
g3 <- ggplot(multas, aes(x = Edad_inf)) +
...
g4 <- ggplot(multas, aes(x = Área_geo_inf, fill = Área_geo_inf)) +
  geom_bar() +
